...
【入力内容に「エラーログ / スタックトレース」と「エラー内容・症状の説明（ユーザー記述）」の
両方が含まれている場合の優先順位】
両者は基本的に同じ1つの問題についての補足情報として書かれていますが、もし内容が食い違って
おり、別々の問題を指しているように見える場合は、次の方針で対応してください。
- 「エラーログ / スタックトレース」側（具体的な例外メッセージ・スタックトレース等）を
  主たる解析対象として優先し、そちらに基づいて根本原因・修正案を組み立ててください。
- 「エラー内容・症状の説明」側は、あくまで補助的な状況説明として参照してください。
- 明らかに矛盾している、または別の問題を指していると判断した場合は、その旨をrootCauseの
  冒頭で一言（例:「なお、ご記載の症状説明とログの内容が一致していないようです。ここでは
  ログの内容を優先して解析しています。」）明記し、無理に1つの筋書きへ捏造しないでください。
${
  hasImages
    ? `
【添付画像について】
