...
- **マルチモーダル入力**: エラーログのテキストに加え、エラー画面のスクリーンショット画像や自由記述の症状説明からも解析可能（画像解析にはAPIキーが必要）。2つ以上を入力した場合、同じ問題の補足情報として統合するか、無関係な別の話題として除外するかをまず判定したうえで、実際に何を優先したかを解析結果の専用ボックスで明示する
...
- **クリップボード監視（試験的機能・デスクトップ版のみ）**: MotionBoard・BigQuery（Google Cloud Console）等、他アプリ/ブラウザで出たエラーメッセージをコピーするだけで自動検知し、貼り付け操作なしで解析欄へ取り込み（オプトインで自動解析まで実行可能）。ヘッダーのボタン自体が実行中は色を変えて状態を表示。組み込みキーワードで拾えない固有の言い回しは、ユーザー自身が「監視ワード」として追加登録可能
- **修正の検証**: 再実行後のログを再解析し、エラーが解消したか／別のエラーが出ていないかを検証。確認チェックリストで自己申告の精度を担保。Gemini解析時は直前に提示した修正案も参考情報として送信し、既に効かなかった案を気づかず繰り返し提案しないようにする
- **トークン消費量の可視化**: Google側の日次クォータ(RPD)がリセットされる太平洋時間の深夜0時を境界に、モデル別のリクエスト数・トークン数をローカルに永続化して表示（アプリ再起動でも本日分は保持）。実際に日次上限超過を検知したモデルがあればアンバー表示で警告
