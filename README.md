...
- **モデル選択**: 解析に使うGeminiモデルをヘッダーから切替可能。APIキー設定時は利用可能なモデル一覧を動的取得（画像/動画/音声生成系・Live/Omni系・Robotics系などこのアプリの用途に合わないモデルは自動除外）
...
│   ├── gemini.ts               # Gemini API 呼び出し（リトライ・モデル一覧取得含む）
